...
import type { StrategyOutcome } from "../../engine/types";
...
export function StrategyWealthBars({ outcomes, recommendedId }: Props) {
    ...
    <div className="h-[280px] -ml-2">
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={data} margin={{ top: 8, right: 12, bottom: 0, left: 12 }}>
          ...
          <Tooltip
            formatter={(v: number, key: string) => [fmt.usd(v), key === "median" ? "median" : key]}
          />
          <Bar dataKey="median" radius={[8, 8, 0, 0]}>
            {data.map((d, i) => (
